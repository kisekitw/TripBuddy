        ...
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "32px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <button style={{ background: C.accent, color: "#fff", border: "none", padding: "10px 24px", borderRadius: 100, fontSize: 13, fontWeight: 500, cursor: "pointer" }}>{t.newTrip}</button>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 20 }}>
            <div onClick={() => { setTrip(SAMPLE_TRIP); setView("editor"); setSelDay(null); }} style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.light}`, padding: 24, cursor: "pointer" }}>
              <div style={{ fontSize: 36, marginBottom: 10 }}>{SAMPLE_TRIP.img}</div>
              <h3 style={{ fontSize: 15, fontWeight: 600, color: C.ink, margin: "0 0 3px" }}>{SAMPLE_TRIP.title}</h3>
              <p style={{ fontSize: 12, color: C.muted, margin: "0 0 8px" }}>{SAMPLE_TRIP.dates} · 16 {t.days}</p>
            ...
            <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, borderRadius: 20, padding: 28, width: 440, maxWidth: "90vw" }}>
              ...
              {impStep === "done" && (
                <div>
                  ...
                  <button onClick={() => { setImpOpen(false); setImpStep("idle"); setTrip(SAMPLE_TRIP); setView("editor"); setSelDay(null); }} style={{ width: "100%", background: C.accent, color: "#fff", border: "none", padding: "12px 0", borderRadius: 100, fontSize: 13, fontWeight: 500, cursor: "pointer" }}>{t.openEditor}</button>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column" }}>
      <div style={{ background: C.card, borderBottom: `1px solid ${C.light}`, padding: "8px 16px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0, flexWrap: "wrap" }}>
        <button onClick={() => setView("trips")} style={{ background: "none", border: "none", fontSize: 13, color: C.muted, cursor: "pointer" }}>{t.back}</button>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "210px 1fr 1fr", minHeight: 0 }}>
        ...
        <div style={{ background: C.card, borderRight: `1px solid ${C.light}`, padding: 8, overflowY: "auto" }}>
          ...
          {days.map((d) => (
            <div key={d.id} onClick={() => setSelDay(d.id)} style={{ padding: "6px 8px", borderRadius: 8, marginBottom: 3, cursor: "pointer", border: `1px solid ${selDay === d.id ? C.accent : C.light}`, background: selDay === d.id ? `${C.accent}08` : "transparent" }}>
